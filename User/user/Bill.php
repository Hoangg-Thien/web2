...
$order_id = $_GET['id'] ?? 0;
$stmt = $conn->prepare("
    SELECT h.*, GROUP_CONCAT(CONCAT(s.product_name, ' (', c.quantity, ' x ', c.unit_price, ')') SEPARATOR '<br>') as order_items
    FROM hoadon h
    LEFT JOIN chitiethoadon c ON h.order_id = c.order_id
    LEFT JOIN sanpham s ON c.product_id = s.product_id
